...
def main():
	oFile = open("data_rmr/shannon_index3.tsv", "w")
	print("program starts ...")
	files = os.listdir("geo_result3")
	for file in files:
		print(file)
		idx = file.split("_")[0]
		print(idx)
		data = []
		mFile = open("geo_result3/" + file, "r")
		for ele in mFile:
...
def join_tables():
	table_org ={}
	file_org = open("data/sample3.tsv","r")
	for ele in file_org:
		data = ele.rstrip().split("\t")
		table_org[data[0]] = [data[3],data[4], data[5]]
	print(table_org)
	file_org.close()
	file_index = open("data_rmr/shannon_index3.tsv", "r")
	oFile = open("data_rmr/shannon_index_scenic3.tsv", "w")
...
def join_category():
	table_org ={}
	file_org = open("data/category3.tsv", "r")
	for ele in file_org:
		print(ele)
		data = ele.rstrip().split("\t")
	...
	print(table_org)
	file_org.close()
	file_index = open("data_rmr/shannon_index_scenic3.tsv", "r")
	oFile = open("data_rmr/shannon_index_scenic_category3.tsv", "w")
	for ele2 in file_index:
		data2 = ele2.rstrip().split("\t")
		print(data2)
		if data2[0] in table_org:
