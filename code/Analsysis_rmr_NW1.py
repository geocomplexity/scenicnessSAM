...
def main():
	oFile = open("data_rmr/shannon_index.tsv", "w")
...
def join_tables():
	...
	for ele in file_org:
		...
		table_org[data[0]] = [data[3],data[4], data[5]]
	print(table_org)
	file_org.close()
	file_index = open("data_rmr/shannon_index.tsv", "r")
	oFile = open("data_rmr/shannon_index_scenic.tsv", "w")
	for ele2 in file_index:
		data2 = ele2.rstrip().split("\t")
		if data2[0] in table_org:
			mString = "\t".join(data2) + "\t" + table_org[data2[0]][0] + "\t" + table_org[data2[0]][1] + "\t" + table_org[data2[0]][2] + "\n"
			print(mString)
			oFile.write(mString)
	oFile.close() 
	file_index.close()
...
def join_category():
	...
	print(table_org)
	file_org.close()
	file_index = open("data_rmr/shannon_index_scenic.tsv", "r")
	oFile = open("data_rmr/shannon_index_scenic_category.tsv", "w")
	for ele2 in file_index:
		data2 = ele2.rstrip().split("\t")
		print(data2)
		if data2[0] in table_org:
...
def join_network():
	df1 = pd.read_csv("data_rmr/shannon_index_scenic_category.tsv", sep = "\t")
	df2 = pd.read_csv("data/nw_index1.tsv", sep="\t")
	print(df2.head())
	merged_df = pd.merge(df1, df2, on='idx')

	# Save the merged dataframe to a new tab-separated file
	merged_df.to_csv('data_rmr/shannon_index_scenic_category_nw1.tsv', sep='\t', index=False)
