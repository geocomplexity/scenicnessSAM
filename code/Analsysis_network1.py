...
def join_network():
	df1 = pd.read_csv("data/shannon_index_scenic_category2.tsv", sep = "\t")
	df2 = pd.read_csv("data/nw_index2.tsv", sep="\t")
	print(df2.head())
	merged_df = pd.merge(df1, df2, on='idx')

	# Save the merged dataframe to a new tab-separated file
	merged_df.to_csv('data/shannon_index_scenic_category_nw2.tsv', sep='\t', index=False)
